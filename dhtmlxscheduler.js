...
scheduler._on_mouse_move = function (a) {
    if (this._drag_mode) {
        ...
        if (!this._drag_pos || b.custom || this._drag_pos.x != b.x || this._drag_pos.y != b.y) {
            ...
            var g = new Date(d - 1),
                h = new Date(c);
            if (this._table_view || g.getDate() == h.getDate() && g.getHours() < this.config.last_hour || scheduler._wa && scheduler._wa._dnd) e.start_date = h, e.end_date = new Date(d), this.config.update_render ? this.update_view() : this.updateEvent(this._drag_id);
...
scheduler.setCurrentView = function (a, b) {
    ...
    if (this.callEvent("onBeforeViewChange", [this._mode, this._date, b, a])) {
        var c = "dhx_cal_data",
            d = this._mode == b && this.config.preserve_scroll ? this._els[c][0].scrollTop : !1;
        ...
        for (var f = this._els.dhx_cal_tab, g = 0; g < f.length; g++) f[g].className = "dhx_cal_tab" + (f[g].getAttribute("name") == this._mode + "_tab" ? " active" : "");
...
scheduler._reset_scale = function () {
    if (this.templates[this._mode + "_date"]) {
        ...
        if( scheduler.config.hasOwnProperty('month_start_day') )
    	    h.setDate(h.getDate()+scheduler.config.month_start_day);
        ...
        if (this._table_view) this._reset_month_scale(b, h, i);
        else {
...
scheduler._reset_month_scale = function (a, b, c) {
    ...
    for (var i = this._colsS.heights = [], k = 0; k <= 7; k++) g[k] = " style='height:" + h + "px; width:" + ((this._cols[k] || 0) - 1) + "px;' ";
    var j = 0;
    this._min_date = c;
        ...
        if (g.rows) {
            for (var h = 0; h < g.rows.length; h++) {
                ...
                if (e[h] * c > this._colsS.height - 22) {
                    for (var i = g.rows[h].cells, k = 0; k < i.length; k++) i[k].childNodes[1].style.height = e[h] * c + "px";
...
scheduler._pre_render_events_line = function (a, b) {
    ...
    for (var c = [], d = [], e = 0; e < a.length; e++) {
        var f = a[e],
            ...
            h = f.end_date.getHours();
        f._sday = this._get_event_sday(f);
        c[f._sday] || (c[f._sday] = []);
        if (!b) {
            f._inner = !1;
            for (var i = c[f._sday]; i.length && i[i.length - 1].end_date <= f.start_date;) i.splice(i.length - 1, 1);
            for (var k = !1, j = 0; j < i.length; j++) if (i[j].end_date.valueOf() < f.start_date.valueOf()) {
                k = !0;
            ...
            if (i.length) i[i.length - 1]._inner = !0;
            if (!k) if (i.length) if (i.length <= i[i.length - 1]._sorder) {
                if (i[i.length - 1]._sorder) for (j = 0; j < i.length; j++) {
                    var l = !1;
                    for (k = 0; k < i.length; k++) if (i[k]._sorder == j) {
                        l = !0;
                        break
                    }
                    if (!l) {
                ...
                l = i[0]._sorder;
                for (j = 1; j < i.length; j++) if (i[j]._sorder > l) l = i[j]._sorder;
                f._sorder = l + 1;
                f._inner = !1
            } else f._sorder = 0;
            i.push(f);
        ...
        if (g < this.config.first_hour || h >= this.config.last_hour) if (d.push(f), a[e] = f = this._copy_event(f), g < this.config.first_hour && (f.start_date.setHours(this.config.first_hour), f.start_date.setMinutes(0)), h >= this.config.last_hour && (f.end_date.setMinutes(0), f.end_date.setHours(this.config.last_hour)), f.start_date > f.end_date || g == this.config.last_hour) a.splice(e, 1), e--
    }
    if (!b) {
...
scheduler._pre_render_events_table = function (a, b) {
    ...
    for (var c = [], d = [
        ...
        var m = d[i._sweek],
            n;
        for (n = 0; n < m.length; n++) if (m[n]._eday <= i._sday) break;
    ...
    if (!(a._sday < 0)) {
        ...
        if (c) {
            var d = a.start_date.getHours() * 60 + a.start_date.getMinutes(),
                ...
                h = Math.floor((c.clientWidth - b) / a._count),
                i = a._sorder * h + 1;
            ...
            if (this.config.cascade_event_display) var k = this.config.cascade_event_count,
                ...
                h = Math.floor(c.clientWidth - b - i - l);
...
scheduler.render_event_bar = function (a) {
    ...
    var e = this.xy.bar_height,
        ...
        h = a._timed ? "dhx_cal_event_clear" : "dhx_cal_event_line",
        i = scheduler.templates.event_class(a.start_date, a.end_date, a);
    i && (h = h + " " + i);
    var k = a.color ? "background-color:" + a.color + ";" : "",
        j = a.textColor ? "color:" + a.textColor + ";" : "",
        l = '<div event_id="' + a.id + '" class="' + h + '" style="position:absolute; top:' + f + "px; left:" + c + "px; width:" + (d - c - 15) + "px;" + j + "" + k + "" + (a._text_style || "") + '">';
...
scheduler._magic_parser = function (a) {
    ...
    for (var d = a.doXPath("//coll_options"), e = 0; e < d.length; e++) {
        ...
        if (g) {
            ...
            for (var h = a.doXPath(".//item", d[e]), i = 0; i < h.length; i++) {
                for (var k = h[i], j = k.attributes, l = {
                    key: h[i].getAttribute("value"),
                    label: h[i].getAttribute("label")
                }, o = 0; o < j.length; o++) {
                    var m = j[o];
    ...
    var q = [];
    b = a.doXPath("//event");
    for (e = 0; e < b.length; e++) q[e] = this.xmlNodeToJSON(b[e]), q[e].text = q[e].text || q[e]._tagvalue, q[e].start_date = this.templates.xml_date(q[e].start_date), q[e].end_date = this.templates.xml_date(q[e].end_date);
    return q
};
